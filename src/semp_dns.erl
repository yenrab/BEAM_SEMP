...
resolve(HostOrIP) when is_list(HostOrIP); is_binary(HostOrIP) ->
    ...
    case semp_facades:inet_parse_address(H) of
        {ok, IP} -> {ok, [IP]};
        {error, _} ->
            IPv4 = case semp_facades:inet_getaddrs(H, inet) of {ok, L4} -> L4; _ -> [] end,
            IPv6 = case semp_facades:inet_getaddrs(H, inet6) of {ok, L6} -> L6; _ -> [] end,
